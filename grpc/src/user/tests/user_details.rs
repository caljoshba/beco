...
use crate::{user::{public_user::PublicUser, user_details::UserDetails}, enums::permission_model_operation::PermissionModelOperation};
...
async fn update_first_name() {
    ...
    let result = user_details.update_first_name(Some(first_name.clone()), &public_user, PermissionModelOperation::UPDATE).await;
...
async fn update_other_names() {
    ...
    let result = user_details.update_other_names(Some(other_names.clone()), &public_user, PermissionModelOperation::UPDATE).await;
...
async fn update_last_name() {
    ...
    let result = user_details.update_last_name(Some(last_name.clone()), &public_user, PermissionModelOperation::UPDATE).await;
